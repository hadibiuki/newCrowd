...
export async function getBankAccountsApi(filter: object = { PageNumber: 1, PageSize: 50 }) {
  const query = toQueryString(filter);

  return await axios.get(USER_PATH + `BankAccounts${query}`);
}

export async function addBankAccountApi(body: object) {
  return await axios.post(USER_PATH + 'BankAccounts', body);
}
