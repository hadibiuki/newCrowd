<template>
  <ui-PageHeading
    ...
    :title="
      data
        ? $t('_zarin_link.reservation_link.edit_page_title')
        : $t('_zarin_link.reservation_link.create_page_title')
    "
    size="lg"
    class="payment-link-add-header"
    has-back
    :loading="formLoadingImage || formLoading || formLoadingEdit || loading"
    :description="$t('_zarin_link.reservation_link.page_description')"
...
const { data, loading } = useGetZarinLinkEventQueryById();
