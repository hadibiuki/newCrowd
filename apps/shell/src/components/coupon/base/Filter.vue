...
import { CouponFilterEnum } from '@/graphql/graphql';
...
const { statusList } = useCouponFilter();
const selectedFilter = ref<string>();
const closer: Ref<() => void> = ref(() => {});
const filterRef = ref();
// show filter selected on label
const filteredValue = (value: CouponFilterEnum) =>
  statusList.value.filter((i: StatusItem) => i.value === value)[0]?.label;
// handle filter selected to update route & selectedFilter
const FilterValues = (value: CouponFilterEnum) => {
...
watch(
  () => route.query,
  () => {
    const status = filteredValue(route.query.status as CouponFilterEnum);
    if (route.query.status && status) {
      selectedFilter.value = `${t('_common.filters.placeholder')}: ${filteredValue(
        route.query.status as CouponFilterEnum
      )}`;
    } else {
      selectedFilter.value = undefined;
    }
