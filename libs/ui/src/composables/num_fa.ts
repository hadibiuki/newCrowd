...
const convertDecimalPart = (decimalPart: string) => {
  ...
  return ' ممیز ' + numFa(decimalPart) + ' ' + decimalSuffixes[decimalPart.length];
};
/**
 * Main function
 * @param input
 ...
 * @constructor
 */
const numFa = (input: number | string) => {
  // Clear Non digits
...
String.prototype.toPersianLetter = function () {
  return numFa(this as string);
};

//@deprecated
Number.prototype.toPersianLetter = function () {
  return numFa(this.toString());
};

String.prototype.numFa = function () {
  return numFa(this.toString());
};

Number.prototype.numFa = function () {
  return numFa(this.toString());
};

export default numFa;
